import React from "react";

export default function InstagramIcon({
  className,
  ...props
}: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlSpace="preserve"
      style={{
        fillRule: "evenodd",
        clipRule: "evenodd",
        strokeLinejoin: "round",
        strokeMiterlimit: 2,
      }}
      viewBox="0 0 30 30"
      className={className}
      aria-label="Instagram"
      role="img"
      {...props}
    >
      <path
        d="M1.5 1.633C-.386 3.592 0 5.673 0 11.995c0 5.25-.916 10.513 3.878 11.752 1.497.385 14.761.385 16.256-.002 1.996-.515 3.62-2.134 3.842-4.957.031-.394.031-13.185-.001-13.587-.236-3.007-2.087-4.74-4.526-5.091C18.89.029 18.778.005 15.91 0 5.737.005 3.507-.448 1.5 1.633Z"
        style={{ fill: "url(#instagram-gradient)", fillRule: "nonzero" }}
        transform="translate(.09 .048) scale(1.24616)"
      />
      <path
        d="M11.998 3.139c-3.631 0-7.079-.323-8.396 3.057-.544 1.396-.465 3.209-.465 5.805 0 2.278-.073 4.419.465 5.804 1.314 3.382 4.79 3.058 8.394 3.058 3.477 0 7.062.362 8.395-3.058.545-1.41.465-3.196.465-5.804 0-3.462.191-5.697-1.488-7.375-1.7-1.7-3.999-1.487-7.374-1.487h.004Zm-.794 1.597c7.574-.012 8.538-.854 8.006 10.843-.189 4.137-3.339 3.683-7.211 3.683-7.06 0-7.263-.202-7.263-7.265 0-7.145.56-7.257 6.468-7.263v.002Zm5.524 1.471a1.063 1.063 0 1 0 0 2.126 1.063 1.063 0 0 0 0-2.126Zm-4.73 1.243a4.55 4.55 0 1 0 .001 9.101 4.55 4.55 0 0 0-.001-9.101Zm0 1.597c3.905 0 3.91 5.908 0 5.908-3.904 0-3.91-5.908 0-5.908Z"
        style={{ fill: "#fff", fillRule: "nonzero" }}
        transform="translate(.09 .048) scale(1.24616)"
      />
      <defs>
        <linearGradient
          id="instagram-gradient"
          x1="0"
          x2="1"
          y1="0"
          y2="0"
          gradientTransform="scale(29.576) rotate(-44.997 .943 .317)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" style={{ stopColor: "#FD5", stopOpacity: 1 }} />
          <stop offset="0.5" style={{ stopColor: "#FF543E", stopOpacity: 1 }} />
          <stop offset="1" style={{ stopColor: "#C837AB", stopOpacity: 1 }} />
        </linearGradient>
      </defs>
    </svg>
  );
}
